...
defining $$\varphi_{KL} = \frac{d_{L\sigma} \ \varphi_{K} +
...
\begin{itemize}
  ...
  \item Separating incoming and outgoing fluid seems artificial. Some
    ...
    not $V > 0$.
  \item \footenote{TODO expliquer local et global}
\end{itemize}
...
simpler since it operates on scalars).

The first idea is to use a simple fixed point method : $\Phi_{n+1} =
...
\begin{table}[!h]
  ...
  \begin{tabular}{|l|c|c|c|c|}
    ...
    $U_{x} = 0$ & 73.23 & 59.08 & 85.51 & 77.00 \\
    \hline
  \end{tabular}
...
$U_{x} = 0$. Indeed, the only difference is that horizontal velocity
is not exactly zero when using $U_{x} = 0$, but it does not noticeably
modify the results.
...
global Bernoulli is $- (\int_{\text{inlet}} V)^{2}/2$, and with local
Bernoulli it is $- (\int_{\text{inlet}} V^{2}) / 2$. A simple
application of Cauchy-Schwartz inequality shows that mean pressure is
higher when using global Bernoulli.

When the boundary conditions do not distinguish incoming and outgoing
fluid, incoming fluid is cold. This means it is evacuated less quickly
than when incoming air is at constant temperature, and the mass flux
is lower.
