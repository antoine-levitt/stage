...
\begin{align}
  ...
  \sum_{L \in \mathcal{N}_K} \mathcal{G}_{KL} &= \int_{K} Ra Pr T \vb{e_{z}} \\
...
order terms is straightforward by defining edge values for the fields
: $$\varphi_{KL} = \frac{\varphi_{K} + \varphi_{L}}{2}.$$
For Laplacians, whose finite volume formulation require the
discretisation of edge gradients, we define $$(\nabla \varphi)_{KL} =
\frac{\varphi_{K} - \varphi_{L}}{d_{KL}},$$ where $d_{KL}$ is the
distance between volume centers $x_{K}$ and $x_{L}$.
...
interface $\sigma_{KL}$. The discrete gradient is also defined
accordingly.
...
Once discretised, the problem is iteratively linearised and solved
(see next section on Newton method). However, as it is, our problem is
ill-posed. Firstly, pressure is determined only up to a constant. When
there are no boundary conditions imposing pressure, this is adressed
by adding an equation, for instance by fixing the pressure at a
particular cell.
...
This problem is inherent in collocated grids, and is present for any
meshing in any dimension, because the expression for the pressure
gradient at a cell does not involve the pressure at this cell, as can
be seen in equation TODO, using the fact that $\sum m_\sigma n_\sigma
= 0$. On staggered grids however, where velocity variables are
stored at cell edges, the pressure gradient is computed on an edge,
and the resulting expression involves two consecutive pressure
variables.
