...
\begin{document}
\author{Antoine Levitt\\Stage de deuxi\`eme ann\'ee
  ENSIMAG\\Laboratoire MSME, Universit\'e de
  Marne-La-Vall\'ee\\Encadr\'e par Robert Eymard et \'Eric
  Ch\'enier\\D\'ebut juillet-mi ao\`ut}
\title{Finite volume method for incompressible Navier-Stokes
  equations, application to a benchmark problem}
\maketitle
\abstract{
...
use the collocated scheme introduced in
...
%PERSONNEL
This internship was completed in the laboratory MSME (Modélisation et
Simulation Multi-Échelles), team TCM (Transport de Chaleur et de
Matière). I was supervised by Robert Eymard (a mathematician) and Éric
Chénier (a physicist). The main objective of the work was to
understand the results of a benchmark which was done in 2007, and gave
a wide variety of results. To understand these, I used the FORTRAN
code developped by Éric Chénier.

The main difficulties in the project were to understand the physical
context (Boussinesq approximation), the numerical method (a finite
volume scheme which uses non-standard approximations of fluxes to
ensure certain properties), the implementation (6000 lines of
FORTRAN). Once this was done, the interpretation of results was quite
straightforward.
...
\item {\bf Finite difference method} approximates fields by their
  values on points, and approximates derivatives. This, while the
  simplest of all methods, does not adapt to meshings other rectangulary .
...
\cite{Pat-80-num,har-65-num,nic-92-ana,cho-97-ana,cho-97-mac}. Collocated
...
conditions were probably used. This work presents the differences
produced by these different boundary conditions.
...
\begin{equation}
  ...
  \vb{f}.
\end{equation}
$\mathbb{T}$ is the visceous stress tensor, and \vb{f} are the
external volumetric forces exerting on the fluid (gravity,
electromagnetic ...).  We need an expression of $\mathbb{T}$ as a
function of the other variables for the model to be complete. The
second term, $\vb{v} \cdot \nabla \vb{v}$, known as the convective
acceleration, is to be understood as $(\vb{v} \cdot \nabla) \vb{v}$,
that is, the ``transport operator'' $(\vb{v} \cdot \nabla) =
\sum_{i=1}^d v_i \frac{\partial}{\partial i}$ applied to $\vb{v}$.
...
$T = T - T_0$ has no effect on this equation.
...
to only two. This adimentionalisation requires the choice for a
characteristic length and temperature difference. These are chosen
according to our problem, see section \ref{physsetup}.

The final form of our equations is then
\begin{align}
  \label{b-mass}
...
of meshes, and not limited to orthogonal ones.
...
fields. For $\varphi = T$ or $\vb{v_{i}}$, $i = 1 \dots d$, we
...
Pressure discretisation is more subtle. We require the discrete
pressure gradient to be the dual of discrete divergence (as defined by
the gradient above). This leads to the discretisation of $p
\vb{n}_{KL}$ as $\frac{d_{K \sigma}}{d_{K L}}(p_L -
p_K)\vb{n}_{KL}$. Note that this gradient is not consistent. It is
however required to obtain discrete versions of kinematic energy
inequalities (for more details, see \cite{che-06-num}).
...
\begin{align}
  ...
  + m_{KL} \frac{d_{K \sigma}}{d_{K L}}(p_L -p_K) \vb{n}_{KL}
...
(\frac{\vb{v}_{K} + \vb{v}_{L}}{2}$) . This is again to ensure that
discrete forms of kinetic energy inequalities can be obtained.
...
to a direct method, which is slower but has no convergence issues.
...
established. Incoming air is cold ($T = 0$), and outgoing air goes away
at established temperature ($T_{y} = 0$).
...
Boundary conditions are summarised in table \ref{table:bc}.
\begin{table}[!h]
  \centering
...
We examine three main variations on these.
...
We fix $A = 10$, $Ra = 10^{5}$, $Pr = 0.71$. This corresponds to air,
and a fixed heating. Unless specified otherwise, boundary conditions
are those of table \ref{table:bc}, and the solver is stationary. The
meshing is rectangular, with smaller cells around zones of interest,
i.e. $x = 0$, $y = 2.5$ and $y = 7.5$. The default grid is
50x300. Clusters are groups of 2x2 cells, with $\lambda =
10^{-5}$. The linear solver uses an iterative bi-conjugate gradient
algorithm with 5 levels of incomplete LU (ILU) preconditioning, except
for some problems which require a direct solver (see section
\ref{sec:stab}). The method was implemented in FORTRAN by
E. Ch\'enier.
...
$U_y = 0$. Indeed, the only difference is that the aspiration at the
boundary layer at the inlet appearing when using local Bernoulli is
slightly larger when using $U_y = 0$ (see figure
\ref{fig:LB-0-0-u-bas}), but it does not noticeably modify the
results.
...
pressure. The resulting profiles, both in profile and velocities, are
given figures \ref{fig:GB-v} and \ref{fig:LB-pv}.


\subsubsection{Profiles}
Figures \ref{fig:p-midwidth} and \ref{fig:v-midwidth} represent
pressures and vertical velocities at mid-width for the variations we
detailed. $U = 0$ rather than $U_{y} = 0$ did not significantly
change the profiles, so we use $U_y = 0$ for these.

Near the inlet there are two cases : with GB, fluid movement is that
of a fluid in a conduct, with constant velocity and linearly
decreasing pressure due to viscosity losses. With LB however, there is
a transition phase where the velocity rises to a constant value. As
such, GB better represents fluid motion in a large domain.

Discriminating fluid at the outlet mainly changes the size of the
recirculation, which explains the bumps near the outlet seen in the
profiles.
\section{Conclusion}
The Boussinesq approximation, combined with the numerical method
presented, gives a way to simulate air flow inside our chimney. The
only modelisation problem concerns boundary conditions at the inlet
and outlet. While our boundary conditions give acceptable results,
simulation of neighbouring air give rise to criticism of our
hypothesis. In particular, while our model uses cold air for the
recirculation, thermic diffusion above the chimney makes incoming air
hotter, and our model should take this into account.

% PERSONNEL
From a personal point of view, this internship was the occasion for me
to learn things from a variety of domains : fluid mechanics, finite
volume method, and FORTRAN. I also gained a broader perspective
on numerical methods for fluid mechanics problems.
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.6\textwidth]{figs/LB-0-0-u-bas}
\caption{Level sets of horizontal velocity $U$ at the inlet using
  $U_{y} = 0$ and local Bernoulli, showing an aspiration. We have
  $U_{\text{max}} = 20$. For $U = 0$, we still have an aspiration, but
  smaller (around $U_{\text{max}} = 7$).}
...
\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{figs/p-midwidth}
\caption{Vertical profile of pressure at mid-width.}
\label{fig:p-midwidth}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{figs/v-midwidth}
\caption{Vertical profile of vertical velocity at mid-width.}
\label{fig:v-midwidth}
\end{figure}
