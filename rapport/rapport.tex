...
The finite volume formulation of the mass conservation equation is
then : for each $K \in \mathcal M_\Omega$,
\begin{equation}
  \sum_{L \in \mathcal{N}_K} \mathcal{F}_{KL} = 0,
\end{equation}
with the mass flux defined as
\begin{equation}
  \mathcal{F}_{KL} = \int_{\sigma_{KL}} \vb{v} \cdot \vb{n}_{KL} \ ds.
\end{equation}

In the same way, momentum and energy equations are written 
\begin{equation}
  \sum_{L \in \mathcal{N}_K} \mathcal{G}_{KL} = 0
\end{equation}
and
\begin{equation}
  \sum_{L \in \mathcal{N}_K} \mathcal{H}_{KL} = 0,
\end{equation}
with the momentum and energy fluxes $\mathcal{G}_{KL}$ and
$\mathcal{H}_{KL}$ defined as follows :
\begin{equation}
